  ...
  return (
    <section id="contact" ref={containerRef} className="py-32 md:py-40 px-6 md:px-12 bg-[#FDF8F3]">
      <div className="max-w-[1400px] mx-auto flex flex-col items-center gap-14 md:gap-16">
        ...
        {/* Heading */}
        <div className="contact-reveal flex flex-col items-center text-center gap-6">
          <div style={{ width: 'clamp(100px, 14vw, 180px)' }}>
          ...
          <div className="flex items-end gap-1">
            ...
            <span className="inline-block w-[0.13em] h-[0.13em] bg-[#FF4F40] rounded-[0.03em] mb-1 ml-1 align-baseline" />
          </div>
        </div>
          ...
          <div className="bg-white rounded-[28px] p-8 md:p-10 border border-[#1E1A2E]/[0.06]">
              ...
              {(['project', 'call'] as const).map((m) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-[10px] text-[14px] font-semibold transition-all duration-200 ${
                    mode === m
                      ? 'bg-white shadow-sm text-[#1E1A2E] border border-[#1E1A2E]/[0.04]'
                      : 'text-[#1E1A2E]/40 hover:text-[#1E1A2E]/60'
                  }`}
                >
            ...
            {mode === 'project' ? (
              submitted ? (
                <div className="flex flex-col items-center justify-center py-16 gap-4">
                  <div className="w-14 h-14 rounded-full bg-[#FF4F40]/10 flex items-center justify-center">
                    <svg className="w-7 h-7 text-[#FF4F40]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <h3 className="font-black text-xl text-[#1E1A2E]">Message sent!</h3>
                  <p className="text-[14px] text-[#1E1A2E]/50 text-center max-w-xs">We'll get back to you within 24 hours.</p>
                </div>
              ) : (
                <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }}>
                  <div>
                    <h3 className="font-black text-[25px] text-[#1E1A2E] leading-tight mb-1">Start a project</h3>
                    ...
                    {[
                      ...
                      <div key={f.id} className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          className="w-full bg-[rgba(253,248,243,0.5)] border border-transparent rounded-xl px-4 py-3.5 text-[#1E1A2E] placeholder:text-[#1E1A2E]/25 focus:ring-2 focus:ring-[#FF4F40]/20 focus:border-[#FF4F40]/20 focus:bg-white transition-all text-[14px] outline-none"
                        />
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {BUDGETS.map((range) => (
                        <button
                          ...
                          className={`relative flex items-center justify-center px-3 py-2.5 rounded-xl border transition-all text-[14px] ${
                            budget === range
                              ? 'bg-[#FF4F40]/[0.06] border-[#FF4F40]/30 text-[#FF4F40] font-medium'
                              : 'bg-[rgba(253,248,243,0.5)] border-transparent text-[#1E1A2E]/50 hover:border-[#FF4F40]/15'
                          }`}
                        >
                          {range}
                        </button>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-[rgba(253,248,243,0.5)] border border-transparent rounded-xl px-4 py-3.5 text-[#1E1A2E] placeholder:text-[#1E1A2E]/25 focus:ring-2 focus:ring-[#FF4F40]/20 focus:border-[#FF4F40]/20 focus:bg-white transition-all text-[14px] resize-none outline-none"
                    />
                  </div>
                  <div className="pt-2">
                    <button
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <h3 className="font-black text-[25px] text-[#1E1A2E] leading-tight mb-1">Book a call</h3>
