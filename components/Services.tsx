...
import LobsterServerSticker from '@/imports/LobsterServerSticker';
import LobsterComputerSticker from '@/imports/LobsterComputerSticker';
...
const SERVICES = [
  {
    ...
    Sticker: LobsterCodingSticker, stickerContainer: { w: 580, h: 420, scale: 0.38 },
  },
  {
    ...
    Sticker: LobsterComputerSticker, stickerContainer: { w: 600, h: 440, scale: 0.36 },
  },
  {
    ...
    Sticker: LobsterServerSticker, stickerContainer: { w: 540, h: 470, scale: 0.36 },
  },
];
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center pt-10 pb-4 md:pt-12 md:pb-6 overflow-hidden">
        ...
        <div className="service-sticker relative transition-transform duration-500 group-hover:scale-105"
          style={{ width: service.stickerContainer.w * service.stickerContainer.scale, height: service.stickerContainer.h * service.stickerContainer.scale }}>
          <div className="relative" style={{ width: service.stickerContainer.w, height: service.stickerContainer.h, transform: `scale(${service.stickerContainer.scale})`, transformOrigin: 'top left' }}>
            <service.Sticker />
          </div>
        </div>
      </div>
      <div className="relative z-10 px-7 pb-8 md:px-9 md:pb-10 pt-4 md:pt-6">
        <h3 className="font-['BN_Rollcall','Anton',sans-serif] text-[28px] md:text-[34px] leading-[1] mb-3" style={{ color: service.textColor }}>{service.title}</h3>
        <p className="font-['Inter'] text-[14px] leading-[1.7] mb-6 opacity-60" style={{ color: service.textColor }}>{service.description}</p>
        <div className="flex flex-wrap gap-2 mb-6">
          {service.capabilities.map((cap) => (
            <span key={cap} className="font-['Inter'] text-[11px] uppercase tracking-[0.06em] px-3 py-1.5 rounded-lg transition-colors duration-300"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-400">
            <span className="font-['Inter'] text-[12px] uppercase tracking-wide font-semibold" style={{ color: service.accent }}>Explore</span>
    ...
    <section id="services" ref={containerRef} className="py-28 md:py-36 px-6 md:px-12 bg-[#FDF8F3]">
      <div className="max-w-[1400px] mx-auto">
        <div className="services-heading parallax-section-heading mb-16 md:mb-20 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            <span className="font-['Inter'] text-[10px] uppercase tracking-[0.1em] text-[#F26044] block mb-3">What We Do</span>
          ...
          <p className="font-['Inter'] text-base text-[#1A1A1A]/50 max-w-sm leading-relaxed">Three pillars. One obsession: building digital products that actually matter.</p>
        </div>
