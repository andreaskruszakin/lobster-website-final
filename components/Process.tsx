      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="process-heading parallax-section-heading mb-10">
          ...
          <h2 className="font-black text-5xl md:text-7xl text-[#1E1A2E] leading-[0.9]">
            How we work<span className="inline-block w-[0.13em] h-[0.13em] bg-[#FF4F40] rounded-[0.03em] ml-[0.06em] align-baseline" />
          </h2>
        </div>
        ...
        <div className="process-card rounded-[28px] px-8 py-12 md:px-12 md:py-14" style={{ backgroundColor: 'rgba(254,155,102,0.1)' }}>
          <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#1E1A2E]/10">
            {STEPS.map((step, i) => (
              <div
                key={i}
                className={`process-col flex flex-col gap-3 py-8 md:py-0 ${i === 0 ? '' : 'md:pl-10'} ${i < STEPS.length - 1 ? 'md:pr-10' : ''}`}
              >
                <span
                  className="font-black leading-none text-[#FF4F40]"
                  style={{ fontFamily: "'BN Sonic', sans-serif", fontWeight: 400, fontSize: 'clamp(2.5rem, 5vw, 3.5rem)' }}
                >
                  {step.num}
                </span>
                <h3 className="font-black text-[#1E1A2E] text-xl md:text-2xl leading-tight">{step.title}</h3>
                <p className="text-[14px] leading-relaxed" style={{ color: 'rgba(30,26,46,0.55)', letterSpacing: '-0.01em' }}>
                  {step.description}
                </p>
              </div>
            ))}
